...
// --- Typing indicator ---
// Server owns the inactivity timer (2.5s). Client sends throttled pings.
// Call stopTyping() explicitly for immediate stops (empty Enter, slash cmds).
...
messageInput.addEventListener("input", () => {
    const now = Date.now();
    if (now - lastTypingPing > 1200) {
        lastTypingPing = now;
        isTyping = true;
        socket.emit("typing", { room: chatType, user: user });
    }
});
...
function showReturnButton(scrollPos) {
    ...
    if (!returnBtn) {
        ...
        returnBtn.addEventListener('click', () => {
            wrapper.scrollTop = returnScrollPos;
            hideReturnButton();
        });
